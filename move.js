...
const AddCards = () => {
    // Add featured carousel cards (only first 5)
    let featured_carousel = document.getElementsByClassName("featured-carousel")[0];
    
    for (let j = 0; j < 5; j++) {
        let card = document.createElement("div");
        card.classList.add("featured-card");
        card.style.backgroundImage = `url(${cards[j].background})`;
        card.style.backgroundColor = `#333`;
        card.setAttribute("data-card-index", j);
        
        // Add hover event to update main image
        card.addEventListener("mouseenter", function() {
            let index = parseInt(this.getAttribute("data-card-index"));
            UpdateFeaturedImage(index);
        });
        
        featured_carousel.appendChild(card);
    }
...
const MakeJumbotron = () => {
    // The top section already uses min-height: 100vh in CSS
    // This function is maintained for compatibility but the height is controlled by CSS
};
...
const UpdateFeaturedImage = (cardIndex) => {
    ...
    let descriptionElement = document.getElementById("featured-description");
    let topSection = document.getElementsByClassName("top")[0];
    
    // Update background image with fade effect
    imageContainer.style.backgroundImage = `url(${card.display_background || card.background})`;
